...
`crit-plus` stdout includes the review file path. Read it. Three comment scopes:
...
2. Reply with what you did: `crit-plus comment --reply-to <id> --author 'Cline' '<what you did>'` (markdown supported).
...
EOF
crit-plus comment --json --file /tmp/replies.json --author 'Cline'
```
...
```bash
crit-plus comment --author 'Cline' '<body>'                       # Review-level
crit-plus comment --author 'Cline' <path> '<body>'                # File-level
crit-plus comment --author 'Cline' <path>:<line> '<body>'         # Line
crit-plus comment --author 'Cline' <path>:<start>-<end> '<body>'  # Line range
crit-plus comment --reply-to <id> --author 'Cline' '<body>'       # Reply (c_… or r_…)
```
