...
allowed-tools: Bash(crit-plus:*), Bash(command ls:*), Read, Edit, Glob
...
Run `crit-plus` **in the background** using `run_in_background: true`:
...
**Do NOT proceed until `crit-plus` completes.** Do NOT ask the user to type anything. Do NOT read the review file early. Wait for the background task to finish — that is how you know the human is done reviewing.
...
4. Reply with what you did: `crit-plus comment --reply-to <id> --author 'Claude Code' '<what you did>'` (reply bodies support markdown)
