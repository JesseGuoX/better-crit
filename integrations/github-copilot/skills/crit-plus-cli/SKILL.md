...
## Authoring and replying with `crit-plus comment`

```bash
# Review-level (general feedback)
crit-plus comment --author 'Copilot' '<body>'

# File-level (whole file, no line numbers)
crit-plus comment --author 'Copilot' <path> '<body>'

# Line (single line or range)
crit-plus comment --author 'Copilot' <path>:<line> '<body>'
crit-plus comment --author 'Copilot' <path>:<start>-<end> '<body>'

# Reply to an existing comment
crit-plus comment --reply-to <id> --author 'Copilot' '<body>'
```
...
]' | crit-plus comment --json --author 'GitHub Copilot'
```
...
```bash
crit-plus comment --json --file /tmp/crit-plus-bulk.json --author 'GitHub Copilot'
```
...
If `crit-plus comment` errors with "comment found in multiple files", IDs collided across files. Disambiguate with `--path`:

```bash
crit-plus comment --reply-to c_a1b2c3 --path src/auth.go --author 'Copilot' 'Fixed the null check'
```
...
```bash
crit-plus comment --plan my-plan-2026-03-23 --reply-to c_a1b2c3 --author 'Copilot' 'Updated the plan'
```

## GitHub PR sync

```bash
crit-plus pull [pr-number]                                    # Fetch PR review comments into the review file
