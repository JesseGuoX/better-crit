...
> If the user invokes Crit Plus by name (for example, "use Crit Plus" or `/crit-plus`), use the `/crit-plus` skill instead — it covers the full review loop. This skill covers CLI operations like `crit-plus comment`, `crit-plus pull/push`, and `crit-plus share`.
...
crit-plus comment --author 'Hermes' '<body>'

# File-level (whole file, no line numbers)
crit-plus comment --author 'Hermes' <path> '<body>'

# Line (single line or range)
crit-plus comment --author 'Hermes' <path>:<line> '<body>'
crit-plus comment --author 'Hermes' <path>:<start>-<end> '<body>'

# Reply to an existing comment
crit-plus comment --reply-to <id> --author 'Hermes' '<body>'
```
...
]' | crit-plus comment --json --author 'Hermes'
```
...
```bash
crit-plus comment --json --file /tmp/crit-plus-bulk.json --author 'Hermes'
```
...
Comment IDs are unique per session, but the same ID can collide across files. If `crit-plus comment` errors with "comment found in multiple files", disambiguate with `--path`:

```bash
crit-plus comment --reply-to c_a1b2c3 --path src/auth.go --author 'Hermes' 'Fixed the null check'
```
...
```bash
crit-plus comment --plan my-plan-2026-03-23 --reply-to c_a1b2c3 --author 'Hermes' 'Updated the plan'
```
