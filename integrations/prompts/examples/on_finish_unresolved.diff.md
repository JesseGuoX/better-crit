...
2. Reply with `crit-plus comment --reply-to <id> --author <name> "..."`
...
{{end}}Address each comment. For each one, reply explaining what you did using `crit-plus comment --reply-to <comment-id> --author <your-name> "<explanation>"`. When done run: `{{.next_round_cmd}}`
{{end}}
