...
{{end}}{{if eq .internal_session_mode "plan"}}Revise the plan to address each comment. To reply to comments, use `crit-plus comment --plan {{.plan_slug}} --reply-to <id> --author <your-name> "<explanation>"`.{{else}}For each comment, make any needed code edit and reply explaining what you did using `crit-plus comment --reply-to <comment-id> --author <your-name> "<explanation>"`.{{end}}{{if .next_round_cmd}}
