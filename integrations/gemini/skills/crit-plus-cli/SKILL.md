...
crit-plus comment --author 'Gemini' '<body>'

# File-level (whole file, no line numbers)
crit-plus comment --author 'Gemini' <path> '<body>'

# Line (single line or range)
crit-plus comment --author 'Gemini' <path>:<line> '<body>'
crit-plus comment --author 'Gemini' <path>:<start>-<end> '<body>'

# Reply to an existing comment
crit-plus comment --reply-to <id> --author 'Gemini' '<body>'
```
...
]' | crit-plus comment --json --author 'Gemini'
```

**Prefer `--file <path>` for any multi-paragraph body.** Shell-quoted JSON breaks the moment a `"body"` string contains a raw newline. Use `write_file` to author the JSON to a temp file, then point crit-plus at it:

```bash
# After write_file /tmp/replies.json:
crit-plus comment --json --file /tmp/replies.json --author 'Gemini'
```
...
```bash
crit-plus comment --reply-to c_a1b2c3 --path src/auth.go --author 'Gemini' 'Fixed the null check'
```
...
Plan reviews (via `crit-plus plan` or the `exit_plan_mode` hook) store the review file in `~/.crit/plans/<slug>/`. **Always pass `--plan <slug>`** — without it, `crit-plus comment` looks in the project root and won't find the comments. The slug is shown in the review feedback prompt.

```bash
crit-plus comment --plan my-plan-2026-03-23 --reply-to c_a1b2c3 --author 'Gemini' 'Updated the plan'
```
</important>
