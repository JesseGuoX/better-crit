#!/usr/bin/env bash
# Bridges the container's external port (BRIDGE_PORT) to crit-plus's loopback
# server on CRIT_PORT. crit-plus binds 127.0.0.1 only by design — no auth, can
...
# crit-plus, and only inside the container's network namespace.
set -euo pipefail
...
if [[ "$BRIDGE_PORT" == "$CRIT_PORT" ]]; then
  # Can't bind the same port twice — shift crit-plus internally.
