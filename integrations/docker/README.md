...
- `~/.crit/reviews/<key>/review.json` — review storage. Persists inside the container; mount a volume at `/root/.crit` if you want reviews to survive `docker rm`.
- Network: nothing, unless you use `crit-plus share` (which talks to the hosted relay at crit.md) or `crit-plus pull` / `crit-plus push` (which uses the `gh` CLI for GitHub PR sync — install it separately if needed).

`crit-plus` manages its own local review daemon. No external database or server
is required; reviews are stored as JSON files.
