// crit-plus plugin for opencode.
//
// 1. Injects crit-plus's "Sharing" instructions into the model's system prompt only
//    when the user has opted into sharing by setting `share_url` in their crit-plus
...
// 2. Toasts when the agent starts a blocking `crit-plus` wait so Attention-style
...
//   - Crit Plus config is read by shelling out to `crit-plus config`, which prints a
...
const { isCritWaitCommand, roundReadyToast } = require("./lib/crit-plus-wait-notify.js") as {
...
async function loadShareURL($: any): Promise<string | null> {
  ...
  try {
    const result = await $`crit-plus config`.quiet()
