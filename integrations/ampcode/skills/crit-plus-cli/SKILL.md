...
crit-plus comment --author 'Amp' '<body>'

# File-level (whole file, no line numbers)
crit-plus comment --author 'Amp' <path> '<body>'

# Line (single line or range)
crit-plus comment --author 'Amp' <path>:<line> '<body>'
crit-plus comment --author 'Amp' <path>:<start>-<end> '<body>'

# Reply to an existing comment
crit-plus comment --reply-to <id> --author 'Amp' '<body>'
```
...
]' | crit-plus comment --json --author 'Amp'
```
...
```bash
crit-plus comment --json --file /tmp/crit-plus-bulk.json --author 'Amp'
```
...
```bash
crit-plus comment --reply-to c_a1b2c3 --path src/auth.go --author 'Amp' 'Fixed the null check'
```
...
```bash
crit-plus comment --plan my-plan-2026-03-23 --reply-to c_a1b2c3 --author 'Amp' 'Updated the plan'
```
