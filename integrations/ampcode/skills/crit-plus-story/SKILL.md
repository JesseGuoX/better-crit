...
Story ingest opens the review UI and exits — it does **not** wait for the human. Run `crit-plus` in the foreground and block until it exits:
...
**Do NOT proceed until `crit-plus` completes.** When it does, return to Step 6. If the user finishes with zero comments, the review is approved — stop the loop and proceed.
