...
> If a plan was just written and the user said `/crit-plus` or `crit-plus`, invoke the `/crit-plus` workflow — do not use this reference skill. This skill covers CLI operations like `crit-plus comment`, `crit-plus pull/push`, and `crit-plus share`.
...
crit-plus comment --author 'Windsurf' '<body>'

# File-level (whole file, no line numbers)
crit-plus comment --author 'Windsurf' <path> '<body>'

# Line (single line or range)
crit-plus comment --author 'Windsurf' <path>:<line> '<body>'
crit-plus comment --author 'Windsurf' <path>:<start>-<end> '<body>'

# Reply to an existing comment
crit-plus comment --reply-to <id> --author 'Windsurf' '<body>'
```
...
```bash
crit-plus comment --reply-to c_a1b2c3 --path src/auth.go --author 'Windsurf' 'Fixed the null check'
```
...
```bash
crit-plus comment --plan my-plan-2026-03-23 --reply-to c_a1b2c3 --author 'Windsurf' 'Updated the plan'
```
