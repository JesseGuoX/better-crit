---
name: crit-plus-story
description: "Author a crit-plus story and continue the interactive review loop only when the user explicitly invokes $crit-plus-story or directly asks you to generate a crit-plus story. Do not infer this skill from generic review, PR, or diff-review requests."
---
...
review cycle as `/crit-plus` (wait → address comments → next round). Invoke it
...
(do **not** run bare `crit-plus story`, which spends `agent_cmd` tokens), then run
bare `crit-plus` to listen for Finish Review.
...
temp file, e.g. `/tmp/crit-plus-story.json`. Do not include `version`,
`generated_at`, `agent`, `base_sha`, `head_sha`, `scope_fingerprint`, or
`coverage` — crit-plus fills those in.
...
Exit 0 means the story was saved (crit-plus opens the browser to show it). **Then continue to Step 5** — do not stop after ingest. Exit 1
...
- `missing` non-empty with `auto_repaired: true` on exit 0: crit-plus already
  back-filled the omissions into `support[]`. Optionally re-author to place
  them deliberately.
- A drift error ("diff changed since prep"): re-run `crit-plus story --prep` and
...
Story ingest opens the review UI and exits — it does **not** wait for the human. Run `crit-plus` and wait until it exits:

```bash
crit-plus
```

`crit-plus` reconnects to the story session's daemon (already started by ingest) and blocks until the user clicks "Finish Review".

If ingest printed a review URL, relay it verbatim:

> **"Crit Plus is open at http://localhost:<port>. Leave inline comments on the diff, then click Finish Review."**
...
**Fallback** (mid-round re-entry or headless workflows): `crit-plus comments` / `crit-plus comments --json`.
...
**Do NOT proceed until the background `crit-plus` task completes.** When it does, return to Step 6. If the user finishes with zero comments, the review is approved — stop the loop and proceed.
...
- It does not run bare `crit-plus story` (LLM via `agent_cmd`) unless the user
  explicitly asks for that path.
- It does not call `crit-plus push` or `crit-plus share` unless the user asks to share.
