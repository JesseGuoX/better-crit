...
the user explicitly invokes `/crit-plus` or directly asks to use Crit Plus. A generic
request to review code, a plan, a diff, a PR, or a page does not count.

Review and revise code changes, plans, live pages (running dev servers, staging URLs), or local HTML files using `crit-plus` for inline comment review.
...
## Step 1: Pass arguments to `crit-plus`

The CLI auto-detects the review mode from its arguments. **Do not ask the user which mode to use.** Pass arguments through:

```
crit-plus <arguments>               # file, dir, URL, .html — CLI auto-detects mode
crit-plus --pr <num|url>            # GitHub PR (range mode)
crit-plus --mr <iid|url>            # GitLab MR (range mode)
crit-plus --range <base>..<head>    # commit range (range mode)
crit-plus                           # no args → branch diff
```
If no arguments, check conversation context:

1. A plan file was written earlier in this conversation → `crit-plus <plan-file>`
2. Otherwise → bare `crit-plus` (branch diff)

<important if="the user wants to open the review from another device — e.g. a phone over Tailscale">
Keep crit-plus on loopback and reverse-proxy in. Same Step 1 args still apply (file, bare `crit-plus`, etc.):

```bash
crit-plus --public-url "https://<machine>.ts.net" --allow-unauthenticated-network --no-open [args…]
# then: tailscale serve --bg --https=443 http://127.0.0.1:<port>
```

- `--public-url` only changes the URL crit-plus prints — it does **not** expose the server.
- `--allow-unauthenticated-network` is required with `--public-url` (even on loopback) and with any non-loopback `--host`. Crit Plus has no auth: anyone who can reach the URL can read the repo and post comments that may trigger agents. Confirm the user wants that blast radius.
- **Do not bind `--host` to a Tailscale/LAN IP** — proxy with `tailscale serve` (or an SSH tunnel). Get `<port>` from crit-plus's startup output, or fix it with `-p`. Relay the URL crit-plus prints (the public one), not localhost.
</important>
...
Run `crit-plus` and wait until it exits:

```bash
crit-plus <plan-file>   # specific file
crit-plus               # git mode
```

If a crit-plus server is already running from earlier in this conversation, `crit-plus` automatically connects to it. Starting from scratch, it spawns the daemon, opens the browser, and blocks until the user clicks "Finish Review".

`crit-plus` prints the review URL on startup (e.g. `Started crit-plus daemon at http://localhost:<port>`). Relay it verbatim:

> **"Crit Plus is open at http://localhost:<port>. Leave inline comments, then click Finish Review."**

**Do NOT proceed until `crit-plus` completes.** Do NOT ask the user to type anything. Do NOT read the review file early. Wait for the command to finish — that is how you know the human is done reviewing.

While `crit-plus` is still running, do not send user-facing progress updates. Treat the wait loop as internal state.
...
**Fallback** (mid-round re-entry, plan hooks, or headless workflows): `crit-plus comments` / `crit-plus comments --json`. Use `crit-plus comments --plan <slug>` for plan-mode reviews.
...
4. Reply with what you did: `crit-plus comment --reply-to <id> --author 'Codex' '<what you did>'` (reply bodies support markdown)
...
Re-emit the revised plan inside `<proposed_plan>...</proposed_plan>` so Crit Plus can review the new version.
</important>
...
**Do NOT proceed until the background `crit-plus` task completes.** When it does, return to Step 3. If the user finishes with zero comments, the review is approved — stop the loop and proceed.
