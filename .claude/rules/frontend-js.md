...
- Use cookies for any setting that should persist across `crit-plus` invocations. Crit Plus defaults to a random port (`port=0`), and localStorage is scoped per origin (scheme + host + **port**), so localStorage settings reset every run. Cookies are host-scoped and survive port changes.
